...
import { useState, useEffect, useTransition, useOptimistic, useRef, useMemo } from "react";
...
export function IssueDetailDrawer({
  ...
  // Optimistic issue snapshot: field-edit handlers patch this synchronously
  // inside startTransition, before awaiting the server action, so selects and
  // display text update the instant the user acts. If the action rejects,
  // React discards the pending patch once the transition settles and the
  // drawer falls back to `issue` (the last server-confirmed value) with no
  // manual rollback needed.
  const [optimisticIssue, applyOptimisticIssue] = useOptimistic<BoardIssue | null, Partial<BoardIssue>>(
    issue,
  ...
  // Newest-first, matching how `commentsList` is already populated from the
  // server (see the issue-sync effect below) — the reducer prepends to keep
  // that order for the provisional comment while its post is in flight.
  //
  // The success path commits the same provisional row into `commentsList`
  // itself (postCommentAction doesn't hand back the persisted row to sync
  // against). That commit happens before this transition is considered
  // settled, so React reapplies this pending patch once more against the
  // now-updated base — the `some(...)` guard keeps that reapplication a
  // no-op instead of prepending a second, duplicate-keyed copy.
  const [optimisticComments, addOptimisticComment] = useOptimistic<any[], any>(
    commentsList,
    (current, pending) => (current.some((c: any) => c.id === pending.id) ? current : [pending, ...current])
  );
  ...
  // `issue` is narrowed non-null above; `optimisticIssue` tracks it exactly
  // except mid-transition, so this is always defined. Drawer body renders
  // from `currentIssue`, not `issue`, so edits to status/priority/type/
  // assignee/summary/description show instantly and fall back to the last
  // server-confirmed value if the action rejects.
  const currentIssue = optimisticIssue ?? issue;

  const currentType = issueTypes.find((t) => t.value === currentIssue.type) || issueTypes[1];
  const currentPriority = priorityIcons[currentIssue.priority] || priorityIcons.MEDIUM;
  const currentStatus = statuses.find((s) => s.value === currentIssue.status) || statuses[0];
  const estimatedHours = typeof estimateHours === "number" ? estimateHours : parseFloat(String(estimateHours)) || 0;
  const dueDateStatus = getDueDateStatus(dueDate, currentIssue.status);
  ...
  const handleSummaryBlur = () => {
    ...
    if (trimmed && trimmed !== issue.summary) {
      startTransition(async () => {
        applyOptimisticIssue({ summary: trimmed });
        const res = await updateIssueFieldAction(issue.id, "summary", trimmed);
        if (res?.error) {
          setSummary(issue.summary);
          showToast(`Could not update summary: ${res.error}`);
          return;
        }
        onUpdateIssue({ ...issue, summary: trimmed });
      });
    } else {
      setSummary(issue.summary);
    }
  ...
  const handleDescriptionSave = () => {
    ...
    if (nextDescription !== (issue.description || "")) {
      startTransition(async () => {
        applyOptimisticIssue({ description: nextDescription });
        const res = await updateIssueFieldAction(issue.id, "description", nextDescription || "");
        if (res?.error) {
          setDescription(issue.description || "");
          showToast(`Could not update description: ${res.error}`);
          return;
        }
        onUpdateIssue({ ...issue, description: nextDescription });
      });
    }
  };
  ...
  const handleWorkflowTransition = () => {
    ...
    if (currentIssue.status === "TO_DO") nextStatus = "IN_PROGRESS";
    else if (currentIssue.status === "IN_PROGRESS") nextStatus = "IN_REVIEW";
    else if (currentIssue.status === "IN_REVIEW") nextStatus = "DONE";
    else if (currentIssue.status === "DONE") nextStatus = "IN_PROGRESS";
  ...
  const handlePrioritySelect = (newPriority: IssuePriority) => {
    ...
    startTransition(async () => {
      applyOptimisticIssue({ priority: newPriority });
      const res = await updateIssueFieldAction(issue.id, "priority", newPriority);
      if (res?.error) {
        showToast(`Could not update priority: ${res.error}`);
        return;
      }
  ...
  const handleTypeSelect = (newType: IssueType) => {
    ...
    startTransition(async () => {
      applyOptimisticIssue({ type: newType });
      const res = await updateIssueFieldAction(issue.id, "type", newType);
      if (res?.error) {
        showToast(`Could not update type: ${res.error}`);
        return;
      }
      onUpdateIssue({ ...issue, type: newType });
    });
  };
  ...
  const handleAssigneeSelect = (assigneeId: string | null) => {
    ...
    const nextAssignee = selectedUser
      ? { id: selectedUser.id, name: selectedUser.name, avatarUrl: selectedUser.avatarUrl }
      : null;
    startTransition(async () => {
      applyOptimisticIssue({ assignee: nextAssignee });
      const res = await updateIssueFieldAction(issue.id, "assigneeId", assigneeId || "");
      if (res?.error) {
        showToast(`Could not update assignee: ${res.error}`);
        return;
      }
      onUpdateIssue({ ...issue, assignee: nextAssignee });
    });
  };
  ...
  const handlePostComment = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      // postCommentAction doesn't hand back the persisted row, so this
      // provisional shape both paints the optimistic list immediately and,
      // on success, becomes the permanent entry committed to commentsList
      // below — matching the shape commentsList already renders (author,
      // body, createdAt, reactions).
      ...
      const res = await postCommentAction(issue.id, text);
      if (res?.error) {
        setCommentInput(text);
        showToast(`Could not post comment: ${res.error}`);
        return;
      }
      setCommentsList((prev) => [pendingComment, ...prev]);
    });
  };
                ...
                {isEditingDescription ? (
                  ...
                  >
                    {currentIssue.description ? (
                      <p className="whitespace-pre-wrap leading-relaxed">{currentIssue.description}</p>
                    ) : (
              ...
              <div className="flex flex-col gap-4 pt-4 border-t border-border">
                ...
                <div className="flex items-center justify-between border-b border-border pb-3">
                  <div className="flex gap-4 text-xs font-bold text-text-subtle">
                    ...
                    >
                      <MessageSquare size={14} /> Comments ({optimisticComments.length})
                    </button>
                    <button
            ...
            <div className="flex flex-col gap-5 p-4 rounded-xl bg-surface-sunken border border-border/60 shrink-0 h-fit">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <button
                  ...
                  className={`w-full h-9 px-3 rounded-lg text-xs font-bold flex items-center justify-center gap-2 shadow-sm transition-all cursor-pointer ${
                    isBlocked && currentIssue.status === "IN_REVIEW"
                ...
                >
                  {isBlocked && currentIssue.status === "IN_REVIEW" && <ShieldAlert size={14} className="text-amber-500" />}
                  <span>
                    {currentIssue.status === "TO_DO"
                      ? "Start Progress"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-2 bg-surface p-2 rounded-lg border border-border">
                  <Avatar name={currentIssue.assignee?.name || "Unassigned"} src={currentIssue.assignee?.avatarUrl} size={24} />
                  <select
